...
def transformuj_raport(input_file, output_file=None):
    ...
    try:
        ...
        # Znak '~' oznacza "odwróć warunek" - zostawiamy wszystko, co NIE zawiera słowa "Suma"
        ...
        print(f"Liczba poprawnych, unikalnych zgłoszeń (bez sum sztucznych): {len(df_long)}")
        
    except FileNotFoundError:
        print(f"BŁĄD: Nie znaleziono pliku '{input_file}'. Upewnij się, że nazwa jest poprawna.")
...
if __name__ == "__main__":
    # Sprawdzamy, czy użytkownik podał nazwę pliku wywołując skrypt
    if len(sys.argv) > 1:
